...
// Access Control
if (!isset($_SESSION['user_id']) || ($user_role !== 'HR' && $user_role !== 'CEO')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_loan') {
        try {
            $emp_id = $_POST['emp_id'];
            $category = $_POST['loan_category'];
            $desc = $_POST['description'];
            $principal = floatval($_POST['principal_amount']);
            $rate = floatval($_POST['interest_rate']); 
            $months = intval($_POST['months_to_pay']);
            $freq = $_POST['deduction_frequency'];
            ...
            if ($principal > 0 && $months > 0) {
                ...
                $stmt->execute([
                    $emp_id, $category, $desc, $principal, $rate, $interest_amount, 
                    $total_payable, $months, $freq, $monthly_amort, 
                    $cutoff_deduction, $total_payable, $start_date
                ]);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_status') {
        ...
        $stmt = $conn->prepare("UPDATE [EmployeeManagementSystem].[dbo].[EmployeeLoans] SET status = ? WHERE loan_id = ?");
        $stmt->execute([$new_status, $loan_id]);
        $message = "Loan status updated to $new_status.";
        $messageType = "success";
    }
}
...
// A. Employees Dropdown
$employees = $conn->query("SELECT emp_id, ac_no, first_name, last_name FROM [EmployeeManagementSystem].[dbo].[Employees] WHERE IsActive = 1 ORDER BY last_name")->fetchAll(PDO::FETCH_ASSOC);

// B. Stats
$totalActive = $conn->query("SELECT COUNT(*) FROM [EmployeeManagementSystem].[dbo].[EmployeeLoans] WHERE status = 'Active'")->fetchColumn();
$totalReceivable = $conn->query("SELECT SUM(remaining_balance) FROM [EmployeeManagementSystem].[dbo].[EmployeeLoans] WHERE status = 'Active'")->fetchColumn();
$loansThisMonth = $conn->query("SELECT COUNT(*) FROM [EmployeeManagementSystem].[dbo].[EmployeeLoans] WHERE MONTH(created_at) = MONTH(GETDATE())")->fetchColumn();

// C. Loan List (Filter Logic)
$filter_emp = $_GET['view_emp'] ?? '';
$filter_status = $_GET['status'] ?? ''; // 'Active', 'Paid', 'Hold', or empty for All

$sqlLoans = "SELECT l.*, e.first_name, e.last_name, e.ac_no 
...
$sqlLoans .= " ORDER BY CASE WHEN l.status='Active' THEN 1 ELSE 2 END, l.start_date DESC";

$stmt = $conn->prepare($sqlLoans);
$stmt->execute($params);
